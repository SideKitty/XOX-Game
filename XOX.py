...
pyfont = PyFont(window, "Animations/x.afont", True)
...
def place(x:int, y:int):
    ...
    position:Tuple[int,int] = (
        x * gridSize[0] + (gridSize[0] / 3),
        y * gridSize[1] + (gridSize[0] / 3))
    pyfont.render(f"E{x}_{y}3",
        "X" if value == 1 else "O", (255,255,255), position)
    pyfont.animateEach(f"E{x}_{y}3", 1, False)
